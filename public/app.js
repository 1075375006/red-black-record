(() => {
  ...
  function renderMatch(match) {
    ...
    const settlement = settle(match, record);
    ...
    $$('.market-switch button', card).forEach(button => { const enabled = button.dataset.market === 'had' ? hasHad : hasHhad; button.disabled = !enabled; button.classList.toggle('active', button.dataset.market === market); button.addEventListener('click', () => { if (!enabled) return; setRecord(match.matchId, { ...(getRecord(match.matchId) || {}), market: button.dataset.market, pick: null, handicapLine: null }); render(); }); });
    const labels = market === 'hhad' ? ['让胜', '让平', '让负'] : ['胜', '平', '负'];
    $$('.pick-buttons button', card).forEach((button, index) => { button.querySelector('.pick-main').textContent = labels[index]; button.classList.toggle('active', record?.market === market && record?.pick === button.dataset.pick); button.addEventListener('click', () => { const currentOdds = market === 'hhad' ? hhad : had; setRecord(match.matchId, { ...(getRecord(match.matchId) || {}), market, pick: button.dataset.pick, handicapLine: market === 'hhad' ? numberOrNull(currentOdds.goalLineValue ?? currentOdds.goalLine) : null, note: $('.note-input', card).value.trim(), updatedAt: new Date().toISOString() }); render(); }); });
    $('.note-input', card).value = record?.note || ''; $('.saved-label', card).textContent = record?.updatedAt ? '已保存 ' + formatSavedTime(record.updatedAt) : '';
    $('.save-button', card).addEventListener('click', () => { const current = getRecord(match.matchId) || { market, pick: null, handicapLine: null }; setRecord(match.matchId, { ...current, market, note: $('.note-input', card).value.trim(), updatedAt: new Date().toISOString() }); render(); });
    const settlementRow = $('.settlement-row', card), settlementText = $('.settlement-text', card), settlementDetail = $('.settlement-detail', card);
    if (!record?.pick) { settlementText.textContent = '还没有留下预测'; settlementDetail.textContent = '选择一个方向开始记录'; } else if (settlement.status === 'red') { settlementRow.classList.add('red'); settlementText.textContent = '红 · 预测命中'; settlementDetail.textContent = settlement.detail; } else if (settlement.status === 'black') { settlementRow.classList.add('black'); settlementText.textContent = '黑 · 预测未中'; settlementDetail.textContent = settlement.detail; } else { settlementRow.classList.add('pending'); settlementText.textContent = '等待赛果'; settlementDetail.textContent = result && !isFinishedResult(result) ? '官方结果尚未结算' : '赛后自动判断红黑'; }
    return fragment;
  }
